...
class SupabaseManager:
    ...
    def init_storage(self):
        ...
        if not SUPABASE_SERVICE_ROLE_KEY:
            print(f"⚠️  SUPABASE_SERVICE_ROLE_KEY not found. Skipping bucket creation.")
            print(f"⚠️  Please ensure bucket '{self.bucket_name}' is created manually in Supabase dashboard.")
            return
    ...
    def verify_user(self, email, password):
        """Verify user credentials. Returns (is_verified, error_message)"""
        try:
            hashed_password = self.hash_password(password)
    ...
    def get_user_by_email(self, email):
        """Get user by email. Returns (user_data, error_message)"""
        try:
            ...
            # Check for common "table not found" errors
            if "relation \"public.users\" does not exist" in error_msg.lower() or "404" in error_msg:
                return None, "Database table 'users' not found. Please run supabase_setup.sql in Supabase SQL Editor."
    ...
    def update_loan_status(self, loan_id, status, remarks):
        ...
        try:
            ...
            result = self.admin_client.table('loans').update(data).eq('id', loan_id).execute()
            
            if not result.data:
                print(f"⚠️  No rows updated for loan {loan_id}. Check permissions or if ID exists.")
                return False
